import HomeHeader from "@/components/HomeHeader";
import Layout from "@/components/Layout"
import axios from "axios"
import Link from "next/link"
import { useEffect, useState } from "react"


export default function Products () {
    const [products, setProducts] = useState([]);
    useEffect(() => {
        axios.get('/api/products').then(response => {
        setProducts(response.data.map(product => ({
            ...product,
            currentImageIndex: 0,
            showDescription: false,
        })));
        })
    }, [])
    const handleImageChange = (productId, direction) => {
        setProducts(products.map(product => {
            if(product._id === productId)
            {
                let newImageIndex = product.currentImageIndex + direction
                if(newImageIndex < 0) newImageIndex = product.images.length - 1;
                if(newImageIndex >= product.images.length) newImageIndex = 0;

                return {...product, currentImageIndex: newImageIndex};
            }
            return product;
        }));
    }
    const toggleDescription = (productId) => {
        setProducts(products.map(product => {
            if(product._id === productId)
            {
                return {...product, showDescription: !product.showDescription}
            }
            return product;
        }))
    }
    return (
        <Layout>
            <HomeHeader/>
            <Link className = "button-primary-version" href={"/products/new"}>Add new product</Link>
            <table className="basic-table-products mt-2 mb-2">
                <thead>
                    <tr>
                        <td>Image</td>
                        <td>Product name</td>
                        <td>Price</td>
                        <td></td>
                    </tr>
                </thead>
                <tbody>
                    {products.map(product => (
                    <>
                        <tr key = {product._id}>
                            <td className="flex justify-center items-center">
                                <button  className = "px-1" onClick={() => handleImageChange(product._id, -1)}> {"<"} </button>
                                <img src = {product.images[product.currentImageIndex]} className = "h-10 max-w-100 justify-center items-center"/>
                                <button className = "px-1" onClick={() => handleImageChange(product._id, 1)}> {">"} </button>
                                
                            </td>
                            <td>{product.title}</td>
                            <td>${product.price}</td>
                            <td>
                                <div className="flex flex-row">
                                    <button className = "button-default " onClick = {() => toggleDescription(product._id)}>
                                        {product.showDescription ? 'Hide' : 'Show'} Description
                                    </button>
                                    
                                    <div className = "flex flex-col">
                                        <Link className = "button-default" href = {`/products/edit/`+product._id}>
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
        </svg>

                                            Edit
                                        </Link>
                                        <Link className = "button-red" href = {`/products/delete/`+product._id}>
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
        <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
        </svg>

                                            Delete
                                        </Link>
                                    </div>
                                </div>
                                
                                
                            </td>
                        </tr>
                        {product.showDescription &&
                            <tr>
                                <td colSpan="4" className="text-justify">Description:<br/>{product.description}</td>
                            </tr>
                        }
                        </>
                    ))}
                </tbody>
            </table>
        </Layout>
    )
}